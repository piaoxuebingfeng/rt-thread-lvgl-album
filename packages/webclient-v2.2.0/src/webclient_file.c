...
int webclient_get_file(const char* URI, const char* filename)
{
    ...
    if (total_length)
    {
        LOG_I("save %d bytes.", total_length);
    }
...
static void newsdata_parse(rt_uint8_t *data)
{
...
/**
 * send GET request and store news into the file.
 *
 * @param URI input server address
 * @param filename store response date to filename
 *
...
int webclient_get_news(const char* URI)
{
    ...
    fd = open(newsfile, O_WRONLY | O_CREAT | O_TRUNC, 0);
    if (fd < 0)
    {
    ...
    if (ptr == RT_NULL)
    {
		...
		LOG_D("session->content_length %d\r\n",session->content_length);
    if (session->content_length < 0)
    {
        while (1)
        {
...
int getnews(int argc,char **argv)
{
    char news_uri[200]={0};

    rt_memset(news_uri,0,200);
    rt_sprintf(news_uri,"%s%s","http://api.tianapi.com/bulletin/index?key=",guiconf_global.apikey);
