...
void lv_100ask_page_manager_set_open_page(lv_obj_t * obj, char *name)
{
    ...
    if (lv_obj_get_child_cnt(obj) == 0)
    {
        ...
        if (page_manager->main_page != obj)
        {
            ...
            lv_page_top_status_bar_create(obj);
        }
    }
...
// ��ȡ��ǰ��ʾ��ҳ��
char lv_100ask_page_manager_get_current_page(lv_obj_t * page_manager,char *name)
{
	LV_ASSERT_OBJ(page_manager, MY_CLASS);
	
	lv_100ask_page_manager_t * page_manager_l = (lv_100ask_page_manager_t *)page_manager;
	       
	lv_ll_t * history_ll = &(page_manager_l->history_ll);
...
static void lv_page_back_btn_create(lv_obj_t * parent)
{
    ...
    lv_style_set_outline_color(&style, lv_palette_main(LV_PALETTE_GREY));
...
static void lv_page_top_status_bar_create(lv_obj_t *parent)
{
    top_status_bar_panel = lv_obj_create(parent);
    lv_obj_set_size(top_status_bar_panel, LV_HOR_RES, 32);
    lv_obj_set_style_radius(top_status_bar_panel,0,LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(top_status_bar_panel,lv_color_make(0,0,0),LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(top_status_bar_panel,LV_OPA_TRANSP,LV_STATE_DEFAULT);
    lv_obj_clear_flag(top_status_bar_panel, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
...
//    lv_obj_set_style_text_color(top_status_time_label,lv_color_make(255,255,255),LV_STATE_DEFAULT);
	  ...
	  lv_img_set_src(top_status_location_icon, &ui_img_iconlocation16_png);
		lv_obj_align(top_status_location_icon, LV_ALIGN_LEFT_MID,0, 0);
	  lv_obj_set_width(top_status_location_icon, LV_SIZE_CONTENT);
	  lv_obj_set_height(top_status_location_icon, LV_SIZE_CONTENT);
		
		
		top_status_location_label = lv_label_create(top_status_bar_panel);
		lv_obj_align_to(top_status_location_label,top_status_location_icon,LV_ALIGN_OUT_RIGHT_MID,10,0);
    lv_label_set_text(top_status_location_label,"ZhengZhou");
    lv_obj_set_style_text_color(top_status_location_label,lv_color_make(255,255,255),LV_STATE_DEFAULT); 
}
...
void lv_page_top_status_set_wifi_label(uint8_t connect_status)
{
    if(top_status_wifi_label)
    {
        ...
        {
            lv_label_set_text(top_status_wifi_label,LV_SYMBOL_WIFI);
            lv_obj_set_style_text_color(top_status_wifi_label,lv_color_make(0,255,0),LV_STATE_DEFAULT);
        }
...
{
    if(top_status_sdcard_label)
    {
        if(sdcard_status ==1)
        {
            lv_obj_set_style_text_color(top_status_sdcard_label,lv_color_make(0,255,0),LV_STATE_DEFAULT);
        }
        else if(sdcard_status ==0)
        {
            lv_obj_set_style_text_color(top_status_sdcard_label,lv_color_make(255,255,255),LV_STATE_DEFAULT);
        }
    }
}
...
{
	if(top_status_location_label && location_str)
	{
		lv_label_set_text(top_status_location_label,location_str);
	}
}
